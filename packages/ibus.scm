      ...
      (modify-phases %standard-phases
                     ...
                     (replace 'build
                              (lambda* (#:key inputs outputs #:allow-other-keys)
                                ...
                                (setenv "PATH" (string-join (list gtk2-bin gettext-bin qtbase-bin qttools-bin gyp-bin jdk-bin which-bin python-bin sed-bin grep-bin findutils-bin coreutils-bin unzip-bin bash-bin (getenv "PATH")) ":"))
                                ...
                                (invoke "python3" "build_mozc.py" "build" "-c" "Release" "unix/ibus/ibus.gyp:ibus_mozc" "unix/emacs/emacs.gyp:mozc_emacs_helper" "server/server.gyp:mozc_server" "renderer/renderer.gyp:mozc_renderer" "gui/gui.gyp:mozc_tool")
                                #t))
                     (replace 'install
                              (lambda* (#:key inputs outputs #:allow-other-keys)
                                (let* ((out (assoc-ref outputs "out"))
                		       ...
                		       (version "2.28.4715.102")))
                                ...
                                (substitute* "out_linux/Release/gen/unix/ibus/mozc.xml"
